...
- [x] AC1: When I click "Run Simulation" from the blast radius side panel, a simulation dialog opens with parameter inputs
- [x] AC2: When I configure a simulation (select failure type: Power Loss / Network Failure / Overheating / Firmware Crash, set radius, and set severity), I can click "Simulate" to run the scenario
- [x] AC3: When the simulation completes, the results panel displays: number of affected devices, estimated cascade risk, projected downtime per device, and an overall impact rating
- [x] AC4: When I view simulation results, affected devices are listed with their individual estimated impact (downtime minutes, risk score change)
- [x] AC5: When I click "Save Simulation", the result is persisted and appears in the simulation history table
- [x] AC6: When I navigate to the simulation history view, I see a table of past simulations with columns: date, origin device, failure type, affected count, risk level, and a "View" action to re-display the results on the map
