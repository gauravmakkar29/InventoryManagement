...
## Acceptance Criteria
- [ ] AC1: When I navigate to `/compliance`, I see a list of compliance items with status filter tabs: Approved, Pending, Deprecated
- [ ] AC2: Each tab shows a count of items in that status (e.g., "Approved (12)")
- [ ] AC3: Each compliance item displays: firmware version, device model, certification name, status badge (color-coded), and vulnerability count
- [ ] AC4: When I select the "Pending" tab, only items with status "Pending" are shown
- [ ] AC5: When I select a certification from the certification filter dropdown (e.g., "IEC 62109"), only items with that certification are shown
- [ ] AC6: When I select a device model from the model filter, results are further narrowed to that model
- [ ] AC7: When no items match the active filters, I see "No compliance items found" with a suggestion to adjust filters
