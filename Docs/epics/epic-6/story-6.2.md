...
- [x] AC1: When I click on a compliance item or its "View Vulnerabilities" button, an expandable panel opens below the item showing its associated vulnerabilities
- [x] AC2: Each vulnerability displays: CVE ID, severity badge (Critical/High/Medium/Low with color coding), affected component, and remediation status (Open/In Progress/Resolved)
- [x] AC3: Vulnerabilities with severity "Critical" are displayed with a red background accent to draw immediate attention
- [x] AC4: When a vulnerability has been resolved, the resolved date is shown
- [x] AC5: When a compliance item has no vulnerabilities, the panel shows "No vulnerabilities recorded"
- [x] AC6: When I am an Admin or Manager, I see a status dropdown on each vulnerability row to change its remediation status
- [x] AC7: When I am a Viewer, I see the remediation status as read-only text (no dropdown)
- [x] AC8: When I change a vulnerability status to "Resolved", the resolved date is automatically set to now
