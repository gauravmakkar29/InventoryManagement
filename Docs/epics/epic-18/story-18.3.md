...
## Acceptance Criteria
- [ ] AC1: When I navigate to the Inventory page and type in the search bar, the device table results are powered by OpenSearch full-text search instead of client-side filtering
- [ ] AC2: When I type a partial device name, serial number, or location, results appear with fuzzy matching (e.g., "sydny" matches "Sydney")
- [ ] AC3: When I apply a status filter (Online/Offline/Maintenance) alongside a text query, results are filtered by both criteria
- [ ] AC4: When I apply a location filter and a model filter together, only devices matching both filters appear
- [ ] AC5: When I filter by health score range (e.g., 0-50), only devices with health scores within the range are displayed
- [ ] AC6: When I clear all filters and search text, the full device list is restored (via standard DynamoDB query)
- [ ] AC7: When OpenSearch is unavailable, the search automatically falls back to DynamoDB GSI queries with an informational message: "Full-text search temporarily unavailable. Using basic search."
