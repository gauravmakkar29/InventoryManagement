...
## Acceptance Criteria
- [ ] AC1: When I navigate to the Playbooks section (tab within Incidents page), I see a list of available playbooks with: name, category, severity match, number of steps, and status (Active/Draft/Deprecated)
...
- [ ] AC3: When I attach a playbook to an incident, the incident detail panel shows a "Playbook Execution" section with a checklist of all steps
- [ ] AC4: When I mark a playbook step as complete, it records who completed it and when, and the progress bar updates
- [ ] AC5: When all playbook steps are completed, a "Playbook Complete" badge appears on the incident and a notification is sent to the incident assignee
- [ ] AC6: When a playbook step has an automated action type (e.g., "isolate_device", "notify_team"), clicking "Execute" on that step triggers the corresponding system action automatically
- [ ] AC7: When I filter playbooks by category, only matching playbooks are shown
