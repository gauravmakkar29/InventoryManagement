...
const sbomQueryKey = queryKeys.sbom.list();
const sbomVulnQueryKey = [...queryKeys.sbom.all, "vulnerabilities"] as const;
...
export function useSBOMData() {
  ...
  const {
    ...
    queryKey: sbomQueryKey,
    queryFn: mockQueryFn(MOCK_SBOMS),
    initialData: MOCK_SBOMS,
  });

  const { data: vulnerabilities = [] } = useQuery({
    queryKey: sbomVulnQueryKey,
  ...
  const handleUpload = useCallback(
    (firmwareId: string, format: SBOMFormat, fileName: string) => {
      try {
        ...
        queryClient.setQueryData<SBOM[]>(sbomQueryKey, (old) => [newSbom, ...(old ?? [])]);
        ...
        setTimeout(() => {
          queryClient.setQueryData<SBOM[]>(sbomQueryKey, (old) =>
            (old ?? []).map((s) =>
              s.id === newSbom.id
                ? {
                    ...s,
  ...
  const handleUpdateVulnStatus = useCallback(
    (vulnId: string, newStatus: RemediationStatus, notes: string) => {
      try {
        queryClient.setQueryData<ComponentVulnerability[]>(sbomVulnQueryKey, (old) =>
          (old ?? []).map((v) =>
            v.id === vulnId
              ? {
                  ...v,
