/**
 * IMS Gen 2 — AWS Amplify Gen2 API Provider
 *
 * Connects to an Amplify Gen2-provisioned AppSync GraphQL API or REST API
 * using the resilient API client. No AWS SDK dependency — just HTTP calls
 * with JWT auth headers.
 *
 * Amplify Gen2 deploys AppSync + DynamoDB under the hood (same as CDK).
 * This provider reads connection info that originates from amplify_outputs.json.
 *
 * Required env vars (from amplify_outputs.json):
 *   VITE_API_ENDPOINT — AppSync endpoint (data.url)
...
function loadApiConfig(): AmplifyApiConfig {
  const endpoint = import.meta.env.VITE_API_ENDPOINT;
  const apiType = (import.meta.env.VITE_API_TYPE ?? "graphql") as "graphql" | "rest";

  if (!endpoint) {
    throw new Error(
      "Amplify API Provider requires VITE_API_ENDPOINT. " +
        "This value comes from amplify_outputs.json (data.url). " +
...
function getAuthToken(): string | null {
  const stored = localStorage.getItem("ims-amplify-auth-session");
...
/**
 * Create an Amplify Gen2 API provider that calls AppSync/API Gateway.
 *
 ...
 * actual AppSync schema generated by Amplify Gen2.
 */
export function createAmplifyApiProvider(): IApiProvider {
  ...
  function notImplemented(method: string): never {
    throw new Error(
      `[Amplify API] ${method} is not yet connected to a live backend. ` +
        `Implement the GraphQL query/mutation in amplify-api-provider.ts.`,
    );
  }
