/**
 * IMS Gen 2 — AWS CDK Auth Adapter
 *
 * Connects to a CDK-provisioned Cognito User Pool using lightweight
 * AWS SDK calls. Does NOT depend on @aws-amplify/* SDK — uses standard
 * OAuth 2.0 / OIDC flows via fetch.
 *
 * Required env vars (see Docs/integration-contract.md):
 *   VITE_AUTH_PROVIDER_URL — Cognito issuer URL
 *   VITE_AUTH_CLIENT_ID    — Cognito app client ID
 *   VITE_AUTH_REGION       — AWS region (default: ap-southeast-2)
 *
 * @see Story #205 — AWS CDK adapter package
 */
...
function loadConfig(): CdkAuthConfig {
  ...
  if (!issuerUrl || !clientId) {
    throw new Error(
      "CDK Auth Adapter requires VITE_AUTH_PROVIDER_URL and VITE_AUTH_CLIENT_ID. " +
...
const STORAGE_KEY = "ims-cdk-auth-session";
const REFRESH_TOKEN_KEY = "ims-cdk-refresh-token";
const MFA_SESSION_KEY = "ims-cdk-mfa-session";

// =============================================================================
// Adapter implementation
// =============================================================================

export function createCdkAuthAdapter(): IAuthAdapter {
  const config = loadConfig();
  let currentRefreshToken: string | null = null;
  let mfaSession: string | null = null;

  // User Pool ID available from issuer URL if needed:
  // const userPoolId = config.issuerUrl.split("/").pop() ?? "";
  ...
  return {
    ...
    async setupMfa(email: string): Promise<string> {
      ...
      // For now, return a placeholder TOTP URI.
      // Full implementation requires AssociateSoftwareToken API call.
      console.warn("[CDK Auth] MFA setup via Cognito API requires access token flow");
    ...
    async confirmMfaSetup(_code: string, _email: string): Promise<void> {
      ...
      console.warn("[CDK Auth] MFA confirm setup requires access token flow");
    },

    isMfaEnabled(_email: string): boolean {
      // Would require AdminGetUser API call — not available from SPA
      // The MFA flow is handled during signIn when Cognito returns a challenge
      return false;
    },
