/**
 * IMS Gen 2 — AWS CDK API Provider
 *
 * Connects to a CDK-provisioned AppSync GraphQL API or REST API Gateway
...
interface CdkApiConfig {
  endpoint: string;
  apiType: "graphql" | "rest";
}

function loadApiConfig(): CdkApiConfig {
  const endpoint = import.meta.env.VITE_API_ENDPOINT;
  const apiType = (import.meta.env.VITE_API_TYPE ?? "graphql") as "graphql" | "rest";

  if (!endpoint) {
    throw new Error(
      "CDK API Provider requires VITE_API_ENDPOINT. See Docs/integration-contract.md.",
    );
  }
...
function getAuthToken(): string | null {
  const stored = localStorage.getItem("ims-cdk-auth-session");
...
/**
 * Create a CDK API provider that calls AppSync/API Gateway.
 *
 ...
 * actual AppSync schema.
 */
export function createCdkApiProvider(): IApiProvider {
  ...
  function notImplemented(method: string): never {
    throw new Error(
      `[CDK API] ${method} is not yet connected to a live backend. ` +
        `Implement the GraphQL query/mutation in cdk-api-provider.ts.`,
    );
  }
